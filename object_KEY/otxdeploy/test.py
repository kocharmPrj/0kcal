    ...
    def detect_objects(self, frame):
        ...
        for i, detection in enumerate(results[0]):
            ...
            if score > 0.5 and class_id in self.labels:
                ...
                label_id = self.labels[class_id]["_id"]
                color = self.labels[class_id]["color"]

                label_val = (label_id, label_name)
                print("", label_val)
                
                detected_objects.append({"label": label_val, "box": (x_min, y_min, x_max, y_max), "color": color})
